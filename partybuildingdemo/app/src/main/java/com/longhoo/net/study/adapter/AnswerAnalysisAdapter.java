...
public class AnswerAnalysisAdapter extends RecyclerView.Adapter {
    ...
    private List<AnswerAnalysisBean.DataBean> dataList;

    public AnswerAnalysisAdapter(Context context, List<AnswerAnalysisBean.DataBean> dataList) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new AnswerViewHolder(LayoutInflater.from(context).inflate(R.layout.item_answer_analysis, parent, false));
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof AnswerViewHolder) {
            ...
            if(TextUtils.isEmpty(dataList.get(position).getAnswerinfo()+"")){
                ((AnswerViewHolder) holder).tvAnalysis.setVisibility(View.GONE);
            }else{
                ((AnswerViewHolder) holder).tvAnalysis.setVisibility(View.VISIBLE);
            ...
            if (type==1) {
                int userAnswer=0;
                try {
                    userAnswer = Integer.parseInt(dataList.get(position).getUser_answer());
                ...
                int []answers=null;
                try {
                    //String [] userAnswers = dataList.get(position).getUser_answer().split("[|]");

                    char[] chars = dataList.get(position).getUser_answer().toCharArray();
                    answers = new int[chars.length];
                    for(int i=0;i<chars.length;i++){
                        answers[i] = Integer.parseInt(Character.toString(chars[i]))-1;
                    }


//                    answers = new int[userAnswers.length];
//                    for(int i=0;i<userAnswers.length;i++){
//                        answers[i] = Integer.parseInt(userAnswers[i])-1;
//                    }
                }catch (Exception e){
                    e.printStackTrace();
                }
