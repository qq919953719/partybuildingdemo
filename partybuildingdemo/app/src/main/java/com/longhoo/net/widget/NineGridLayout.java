...
public  class NineGridLayout extends ViewGroup {
    ...
    public NineGridLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        mSpacing = DisplayUtil.dp2px(context,4);
        init(context);
    }
    ...
    private void refresh() {
        ...
        if (size == 1) {
            ...
            params.width = Utils.getDeviceSize(mContext).x- DisplayUtil.dp2px(mContext,30);
            params.height = (Utils.getDeviceSize(mContext).x-DisplayUtil.dp2px(mContext,30))*3/4;
            setLayoutParams(params);
            imageView.layout(0, 0, Utils.getDeviceSize(mContext).x-DisplayUtil.dp2px(mContext,30),(Utils.getDeviceSize(mContext).x-DisplayUtil.dp2px(mContext,30))*3/4);
    ...
    private void layoutParams() {
        int singleHeight = mSingleWidth*3/4;
    ...
    private void layoutImageView(ImageView imageView, int i, String url, boolean showNumFlag) {
        ...
        int singleHeight = singleWidth*3/4;
...
//        params.width = width;
     ...
     */
    protected boolean displayOneImage(final ImageView imageView, String url, int parentWidth){
        LayoutParams lp = new LayoutParams(LayoutParams.MATCH_PARENT,(Utils.getDeviceSize(mContext).x-DisplayUtil.dp2px(mContext,30))*2/3);
        imageView.setLayoutParams(lp);
        ImageLoader.getInstance().displayImage(url, imageView,getPhotoImageOption(),new SimpleImageLoadingListener(){
            @Override
            public void onLoadingComplete(String imageUri, View view, Bitmap loadedImage) {
                super.onLoadingComplete(imageUri, view, loadedImage);
                int newW = Utils.getDeviceSize(mContext).x-DisplayUtil.dp2px(mContext,30);
                int newH = newW*3/4;
    ...
    protected  void displayImage(ImageView imageView, String url){
        ImageLoader.getInstance().displayImage(url, imageView,getPhotoImageOption());
    }

    protected  void onClickImage(int position, String url, List<String> urlList){
        if (onClickImageListenr != null) {
            onClickImageListenr.onClickImage(position);
        }
    }

    private OnClickImageListenr onClickImageListenr;
    public void setOnClickImageListener(OnClickImageListenr onClickImageListener){
        this.onClickImageListenr = onClickImageListener;
    }
