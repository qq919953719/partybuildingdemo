...
import com.longhoo.net.study.bean.StudyItemDetailBean;
 ...
 * 学习交流-在线学习-列表-详情
 */
public class StudyItemDetailActivity extends BaseActivity implements HttpRequestView {
    ...
    protected int getContentId() {
        return R.layout.activity_sudy_item_detail;
    }
    ...
    protected void initViews() {
        ...
        url = Consts.BASE_URL + "/Onlinestudy/log_newsinfo";
    ...
    public void onRefreshSuccess(String response) {
        ...
        Gson gson = new Gson();
        try {
            StudyItemDetailBean detailsBean = gson.fromJson(response, StudyItemDetailBean.class);
            if (!TextUtils.equals(detailsBean.getCode(), "0")) {
                ToastUtils.getInstance().showToast(this, "获取数据失败~");
                return;
            }
            //更新UI
            StudyItemDetailBean.DataBean newsBean = detailsBean.getData();
            titleText = newsBean.getTitle() + "";
            tvMainTitle.setText(titleText);
            tvTime.setText(newsBean.getTime() + "");
            videoUrl = newsBean.getVideourl();
            thumbUrl = newsBean.getVideoimg();
