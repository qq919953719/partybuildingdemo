...
public class X5WebView extends WebView {
	...
	private WebChromeClient chromeClient = new WebChromeClient() {
		...
		public boolean onJsAlert(WebView arg0, String arg1, String arg2,
			...
			// arg3.confirm();
			// return true;
			ULog.i("yuanhaizhou", "setX5webview = null");
		...
		public void onReceivedTitle(WebView arg0, final String arg1) {
			...
			ULog.i("yuanhaizhou", "webpage title is " + arg1);
		...
		this.getView().setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_DOWN:
					...
					xbefore = MotionEventCompat.getX(event, pointerIndex);
					ybefore = MotionEventCompat.getY(event, pointerIndex);
					break;

				case MotionEvent.ACTION_MOVE: {
					int pointerIndexM = MotionEventCompat.getActionIndex(event);
					xafter = MotionEventCompat.getX(event, pointerIndexM);
					yafter = MotionEventCompat.getY(event, pointerIndexM);
					break;
				}

				case MotionEvent.ACTION_UP:
					double movey = Math.abs(yafter - ybefore);
					double movex = Math.abs(xafter - xbefore);
					if (movex < 10 && movey < 10) {
//						System.out.println("--------------->event.getAction>>"
//								+ "y:" + movey + "x:" + movex);
						HitTestResult hitTestResult = webview
								.getHitTestResult();
						final String path = resourceUrl = hitTestResult
								.getExtra();
						int type = hitTestResult.getType();
						if (type == HitTestResult.IMAGE_TYPE
								|| type == HitTestResult.SRC_IMAGE_ANCHOR_TYPE) {
//							Intent intent = new Intent(arg0,
//									PhotoSwitchActivity.class);
//							intent.putExtra(
//									PhotoSwitchActivity.PHOTOSWITCH_NAME, path);
//							intent.putExtra(
//									PhotoSwitchActivity.PHOTOSWITCH_CURSEL, 0);
//							intent.putExtra(
//									PhotoSwitchActivity.PHOTOSWITCH_URL, "");
//							if (isImgLook) {
...
//	}
	...
	public boolean tbs_dispatchTouchEvent(MotionEvent ev, View view) {
		...
		ULog.d("Bran", "dispatchTouchEvent " + ev.getAction() + " "
				+ r);
		return r;
	}
