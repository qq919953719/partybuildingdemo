...
public class SuperFileView2 extends FrameLayout implements TbsReaderView.ReaderCallback {
    ...
    public void displayFile(File mFile) {
        ...
        if (mFile != null && !TextUtils.isEmpty(mFile.toString())) {
            ...
            if (!bsReaderTempFile.exists()) {
                ULog.d("准备创建/storage/emulated/0/TbsReaderTemp！！");
                boolean mkdir = bsReaderTempFile.mkdir();
                if(!mkdir){
                    ULog.e("创建/storage/emulated/0/TbsReaderTemp失败！！！！！");
                }
            }

            //加载文件
            Bundle localBundle = new Bundle();
            ULog.d(mFile.toString());
    ...
    private String getFileType(String paramString) {
        ...
        if (TextUtils.isEmpty(paramString)) {
            ULog.d(TAG, "paramString---->null");
            return str;
        }
        ULog.d(TAG, "paramString:" + paramString);
        ...
        ULog.d(TAG, "paramString.substring(i + 1)------>" + str);
        return str;
    }
